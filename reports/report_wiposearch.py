...
def send_search(reportType=None):
    ...
    if reportType == 'new':
        ...
        query = "select status from etl_db.reports_status where proc_date='{}' and report_name='report_wiposearch'".format(
            proc_date)
    ...
    query0 = """
              ...
              where tm.InternationalMarkIdentifier <> '-'
    ...
    END as Text3
    ,c.proc_date,c.InternationalMarkIdentifier,g.*,h.intregn as withoutactionhistory
    ...
    left outer join ipv_db.with_actionhistory g
    on
    c.InternationalMarkIdentifier=g.intregn
    left outer join ipv_db.without_actionhistory h
    on
    c.InternationalMarkIdentifier=h.intregn
    ...
    df_data.columns = ['mailertype', 'st13applicationnumber', 'fileid', 'extensioncounter', 'cardtype',
                       ...
                       'internationalregistrationnumber', 'intregn', 'img_text', 'name', 'address1', 'address2',
                       'address3', 'address4', 'country', 'docid', 'notdate', 'link', 'basappn', 'basregn', 'withotAH']
    ...
    df_final = df_final.loc[:,
               ['mailertype', 'st13applicationnumber', 'fileid', 'extensioncounter', 'cardtype', 'legalentityname',
                ...
                'text1_tmnumber', 'text2_status', 'text3', 'proc_date', 'internationalregistrationnumber', 'intregn',
                'img_text', 'name', 'address1', 'address2', 'address3', 'address4', 'country', 'docid', 'notdate',
                'link', 'basappn', 'basregn', 'withoutAH']]
    ...
    df_final.loc[pd.notnull(df_final['withoutAH']), 'withoutAH'] = 'Yes'
    df_final = df_final.drop('city', axis=1)
    df_final.to_excel('wipo_unrepresented.xlsx', index=None, engine='openpyxl')
    print(df_final.shape)
    #############################################################################
    ...
    def get_mail_params(rfile):
        ...
        var_params = {
            'text': '',
            'subject': 'CIPO/WIPO unrepresented searched: {} {}'.format(td.strftime("%B"), td.strftime("%Y")),
            'attach_file': rfile,
        }
    ...
    send_mail(get_mail_params('wipo_unrepresented.xlsx'))
    print('email sent')

    query = "update etl_db.reports_status set status=1 where proc_date='{}' and report_name='report_wiposearch'".format(
        proc_date)
    impala_cur.execute(query)
...
if __name__ == "__main__":
    try:
        print('=' * 30)
        print('Getting CIPO/WIPO unrepresented searched')
        print('=' * 30)
        try:
