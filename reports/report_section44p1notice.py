...
def send_search(reportType=None):
    ...
    if reportType == 'new':
        ...
        query = "select status from etl_db.reports_status where proc_date='{}' and report_name='report_section44p1notice'".format(
            proc_date)
    ...
    query1 = """
             ...
             and b.markeventcode in (244,179,280,61)       
             """
    ...
    WHEN c.applicationlanguagecode = 'en' AND (int_months_between(b.markeventresponsedate,NOW()) BETWEEN 0 AND 2) THEN 'URS3_E'
    WHEN c.applicationlanguagecode = 'fr' AND (int_months_between(b.markeventresponsedate,NOW()) BETWEEN 0 AND 2) THEN 'URS3_F'
    WHEN c.applicationlanguagecode = 'en' AND (int_months_between(b.markeventresponsedate,NOW()) BETWEEN 3 AND 4) THEN 'URS2_E'
    WHEN c.applicationlanguagecode = 'fr' AND (int_months_between(b.markeventresponsedate,NOW()) BETWEEN 3 AND 4) THEN 'URS2_F'
    WHEN c.applicationlanguagecode = 'en' AND (int_months_between(b.markeventresponsedate,NOW()) >= 5) THEN 'URS1_E'
    WHEN c.applicationlanguagecode = 'fr' AND (int_months_between(b.markeventresponsedate,NOW()) >- 5) THEN 'URS1_F'
    ELSE ''
    END as CardType
    ...
    ,CASE
    WHEN c.applicationlanguagecode = 'en' THEN 'STATUS: FAILURE TO COMPLY WITH NICE CLASSIFICATION'
    WHEN c.applicationlanguagecode = 'fr' THEN 'STATUS: DÉFAUT DE FOURNIR UNE DÉCLARATION EN VERTU DU PARAGRAPHE 44.1(1)'
    END as Text2_Status
    ,CASE
    ...
    from (
          select * from ipv_db.tmp2 where st13applicationnumber not in (select distinct st13applicationnumber from ipv_db.tmp2 where markeventcode <> 244)
         ) as a 
    inner join ipv_db.ca_tm_mark_event b
    on
    ...
    where a.markeventcode in (244) and markeventresponsedate<>'-' and markeventresponsedate<>' '
    ...
    df_final.to_excel('section44p1.xlsx', index=None, engine='openpyxl')
    print(df_final.shape)
    #############################################################################
    ...
    def get_mail_params(rfile):
        ...
        var_params = {
            'text': '',
            'subject': 'Section 44.1 Notice unrepresented searched: {} {}'.format(td.strftime("%B"), td.strftime("%Y")),
            'attach_file': rfile,
        }
    ...
    send_mail(get_mail_params('section44p1.xlsx'))
    print('email sent')

    query = "update etl_db.reports_status set status=1 where proc_date='{}' and report_name='report_section44p1notice'".format(
        proc_date)
    impala_cur.execute(query)
...
if __name__ == "__main__":
    try:
        print('=' * 30)
        print('Getting Section 44.1 Notice unrepresented searched')
        print('=' * 30)
        try:
