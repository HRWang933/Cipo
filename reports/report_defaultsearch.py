...
def send_defaultsearch(reportType=None):
    ...
    if reportType == 'new':
        ...
        query = "select status from etl_db.reports_status where proc_date='{}' and report_name='report_defaultsearch'".format(
            proc_date)
    ...
    query0 = """
              ...
              and nti.markcurrentstatusinternaldescriptiontext= 'Default - Searched'
              ...
              and tm.st13applicationnumber in (select distinct(me.st13applicationnumber) from ipv_db.ca_tm_mark_event me where me.markeventcode in(13,14));
    ...
    query1 = """
             ...
             where markeventcode in (20,22,13,18,12,13,14,23)
    ...
    query2 = """
    select distinct 'DEFAULT-SEARCHED' as MailerType,a.st13applicationnumber,substr(cast(a.st13applicationnumber as string),7,7) as Fileid
    ,substr(cast(a.st13applicationnumber as string),14,2) as ExtensionCounter
    ,CASE
    
    WHEN c.applicationlanguagecode = 'en' AND (int_months_between(b.markeventresponsedate,NOW()) BETWEEN 0 AND 2) THEN 'UD3_E_1'
    WHEN c.applicationlanguagecode = 'fr' AND (int_months_between(b.markeventresponsedate,NOW()) BETWEEN 0 AND 2) THEN 'UD3_F_1'
    WHEN c.applicationlanguagecode = 'en' AND (int_months_between(b.markeventresponsedate,NOW()) BETWEEN 3 AND 4) THEN 'UD2_E_1'
    WHEN c.applicationlanguagecode = 'fr' AND (int_months_between(b.markeventresponsedate,NOW()) BETWEEN 3 AND 4) THEN 'UD2_F_1'
    WHEN c.applicationlanguagecode = 'en' AND (int_months_between(b.markeventresponsedate,NOW()) >= 5) THEN 'UD1_E_1'
    WHEN c.applicationlanguagecode = 'fr' AND (int_months_between(b.markeventresponsedate,NOW()) >- 5) THEN 'UD1_F_1'
    ELSE ''
    END as CardType
    ...
    ,CASE
    WHEN c.applicationlanguagecode = 'en' THEN "STATUS: DEFAULT - SEARCHED"
    WHEN c.applicationlanguagecode = 'fr' THEN "STATUS: EN DÉFAUT – A FAIT L'OBJET D'UNE RECHERC"
    END as Text2_Status
    ,CASE
    WHEN c.applicationlanguagecode = 'en' THEN concat("CIPO RESPONSE DEADLINE TO NOTICE OF DEFAULT SENT: ",from_unixtime(unix_timestamp(b.markeventresponsedate),'MMM'),' ',from_unixtime(unix_timestamp(b.markeventresponsedate),'dd'),' ',from_unixtime(unix_timestamp(b.markeventresponsedate),'yyyy'))
    
    WHEN c.applicationlanguagecode = 'fr' THEN concat("DATE LIMITE DE RÉPONSE DE L OPIC À LA NOTIFICATION DU DÉFAUT ENVOYÉE: ",from_unixtime(unix_timestamp(b.markeventresponsedate),'MMM'),' ',from_unixtime(unix_timestamp(b.markeventresponsedate),'dd'),' ',from_unixtime(unix_timestamp(b.markeventresponsedate),'yyyy'))
    END as Text3
    ,c.proc_date
    ...
    where markeventcode in (20,22,13,18,12,13,14,23)  and markeventresponsedate<>'-' and markeventresponsedate<>' '
    ...
    df_final.to_excel('report_default_searched.xlsx', index=None, engine='openpyxl')
    #############################################################################
    ...
    def get_mail_params(rfile):
        ...
        var_params = {
            'text': '',
            'subject': 'CIPO Unrepresented - Default Searched Report: {} {}'.format(td.strftime("%B"),
                                                                                    td.strftime("%Y")),
            'attach_file': rfile,
        }
    ...
    send_mail(get_mail_params('report_default_searched.xlsx'))
    print('email sent')

    query = "update etl_db.reports_status set status=1 where proc_date='{}' and report_name='report_defaultsearch'".format(
        proc_date)
    impala_cur.execute(query)
...
if __name__ == "__main__":
    try:
        print('=' * 30)
        print('Getting CIPO Unrepresented - Default Searched Report')
        print('=' * 30)
        try:
            send_defaultsearch(sys.argv[1])
        except:
            send_defaultsearch()
