...
def send_search(reportType=None):
    ...
    if reportType == 'new':
        ...
        query = "select status from etl_db.reports_status where proc_date='{}' and report_name='report_search'".format(
            proc_date)
    ...
    WHEN c.applicationlanguagecode = 'fr' THEN concat("DATE LIMITE DE R�PONSE DE L'OPIC: ",from_unixtime(unix_timestamp(b.markeventresponsedate),'MMM'),' ',from_unixtime(unix_timestamp(b.markeventresponsedate),'dd'),' ',from_unixtime(unix_timestamp(b.markeventresponsedate),'yyyy'))
    END as Text3
    ,c.proc_date
    ...
    df_final.to_excel('report_searched.xlsx', index=None, engine='openpyxl')
    print(df_final.shape)
    #############################################################################
    ...
    def get_mail_params(rfile):
        ...
        var_params = {
            'text': '',
            'subject': 'CIPO Unrepresented - Searched Report: {} {}'.format(td.strftime("%B"), td.strftime("%Y")),
            'attach_file': rfile,
        }
    ...
    send_mail(get_mail_params('report_searched.xlsx'))
    print('email sent')

    query = "update etl_db.reports_status set status=1 where proc_date='{}' and report_name='report_search'".format(
        proc_date)
    impala_cur.execute(query)
...
if __name__ == "__main__":
    try:
        print('=' * 30)
        print('Getting CIPO Unrepresented - Searched Report')
        print('=' * 30)
        try:
