...
def send_cancellation(reportType=None):
    ...
    if reportType == 'new':
        ...
        query = "select status from etl_db.reports_status where proc_date='{}' and report_name='report_cipo_cancellation'".format(
            proc_date)
    ...
    query0 = """
              ...
              cpe.MarkEventDate,cpe.MarkEventCode,cpe.markeventdescriptiontext
              from ipv_db.ca_tm_feature mf
              left join ipv_db.ca_tm_cancellation_proceedings cp
              on mf.st13applicationnumber=cp.st13applicationnumber and mf.proc_date=cp.proc_date
              ...
              where (cp.plaintiff_rep_entityName  in ('BAYO ODUTOLA','KAREN HANSEN','OLLIP P.C.') or cp.plaintiff_cor_entityName  in ('BAYO ODUTOLA','KAREN HANSEN','OLLIP P.C.') or nr.EntityName  in ('BAYO ODUTOLA','KAREN HANSEN','OLLIP P.C.')  )
              and nti.markcurrentstatusinternaldescriptiontext in ('Registered')
              and cp.OppositionCaseTypeDescription_en  = 'Section 45 Case'
              and cpe.MarkEventDate > date_sub(now(), interval 3 months)
              and cpe.MarkEventDate < date_add(now(), interval 2 months)
              order by FileID,cpe.MarkEventDate
    ...
    df_data.to_excel('report_cancellation_history.xlsx', index=None, engine='openpyxl')
...
<body>
  ...
  {}
    ...
    file = open("report_cipo_cancellation.html", "w")
    ...
    def get_mail_params(table_html):
        ...
        var_params = {
            'text': table_html,
            'subject': 'CIPO Cancellation History Report: {} {}, {}'.format(td.strftime("%B"), td.strftime("%d"),
                                                                            td.strftime("%Y")),
            'attach_file': '',
            'as_table': False,
            'table_html': '',
    ...
    query = "update etl_db.reports_status set status=1 where proc_date='{}' and report_name='report_cipo_cancellation'".format(
        proc_date)
    impala_cur.execute(query)
...
if __name__ == "__main__":
    try:
        print('=' * 30)
        print('Getting CIPO Cancellation History Report')
        print('=' * 30)
        try:
